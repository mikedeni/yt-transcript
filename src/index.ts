...
app.get("/", () => "Welcome to YouTube Transcript API")
    .post(
        "/transcript",
        async ({ body: { url } }): Promise<APIResponse<TranscriptResponse>> => {
            try {
    ...
    .post(
        "/chat",
        async ({ body: { prompt } }): Promise<APIResponse<ChatResponse>> => {
            try {
    ...
    .post(
        "/summarize",
        async ({ body: { url } }): Promise<APIResponse<ChatResponse>> => {
            try {
                const transcript = await youtubeService.getTranscript(url);
